...
### ACC-001 — 사용자당 최대 3개 계좌

- **필수 계약:** 한 사용자는 일반계좌를 최대 3개까지만 보유할 수 있다. 동시 개설 요청으로도 3개를 초과할 수 없다.
- **테스트 수준:** `I`, `C`
- **핵심 시나리오:**
  - Given 2개 계좌, When 1개를 개설하면, Then 3개가 된다.
  - Given 3개 계좌, When 추가 개설하면, Then 거부하고 계좌번호 registry도 증가하지 않는다.
  - Given 2개 계좌, When 2개를 동시에 개설하면, Then 최종 계좌 수는 3개 이하다.
- **검증 증거:** 사용자 계좌 수, registry 행 수, 예외, rollback.
- **현재 상태:** 개설 시 `User` 행을 잠그고 개수를 확인하여 동시 요청을 직렬화한다.
- **현재 테스트:** `AccountOpeningIntegrationTest` (순차 개설 한도·롤백). 동시 개설 검증 테스트는 현재 없다.

### 시작 자금 — 사용자당 1회 1억원

- **필수 계약:** 첫 KRW 일반계좌에 100,000,000원을 지급하고 추가 계좌는 0원으로 개설한다. USD 계좌를 먼저 개설해도 자격을 유지하며, 기존 KRW 계좌 보유자에게 재지급하지 않는다.
- **검증 증거:** 사용자별 지급 여부, 계좌 잔액, `DEPOSIT` 원장의 금액과 전후 잔액. 동시 최초 개설에서 한 번만 지급하고, 원장 실패 시 지급 이력·계좌·registry까지 롤백한다. 지급 계좌를 삭제한 뒤에도 사용자 이력은 유지한다.
- **검증 공백:** 시작 자금 정책 전용 테스트는 제거되었으며, 위 시나리오는 현재 자동 회귀 테스트로 검증하지 않는다.
...
### INV-001 — 사용자당 최대 3개 투자계좌

- **필수 계약:** 한 사용자는 투자계좌를 최대 3개까지만 보유할 수 있고 동시 요청도 상한을 넘을 수 없다.
- **테스트 수준:** `I`, `C`
- **검증 증거:** 투자계좌 수, registry 행, 통화별 예수금 행, rollback.
- **현재 상태:** 일반계좌와 같이 `User` 행을 잠근 뒤 개수를 확인한다.
- **현재 테스트:** `InvestmentOpeningIntegrationTest` (순차 개설 한도·롤백). 동시 개설 검증 테스트는 현재 없다.
