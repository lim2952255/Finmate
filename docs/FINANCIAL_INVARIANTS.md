...
- **[필수 불변식]** 일반계좌와 증권계좌는 각각 사용자당 최대 3개까지 신규 개설한다. 일반계좌는 통화를 합산한다. 기존 상한 초과 계좌를 삭제하거나 잔액을 보정하지 않는다.
- **[필수 불변식]** 모의 시작 자금은 사용자당 최초 1회, 첫 KRW 일반계좌에 100,000,000원을 지급한다. USD 일반계좌와 증권계좌 및 이후 계좌는 0으로 시작한다. USD를 먼저 개설해도 지급 자격을 소진하지 않는다.
- **[현재 구현 사실]** 개설 서비스는 먼저 `User`를 `PESSIMISTIC_WRITE`로 조회하고 개수·지급 이력을 확인한다. 계좌·계좌번호 registry·사용자 지급 이력·시작 자금 `DEPOSIT` 원장은 같은 트랜잭션으로 저장한다. 기존 이체 경로의 잠금 순서는 유지한다.
- **[현재 구현 사실]** `User.simulationFundingGranted`는 계좌와 독립적으로 유지된다. 이전 정책의 KRW 계좌가 남아 있는 사용자는 다음 일반계좌 개설 시 지급 완료로 기록하고 추가 지급하지 않는다. 배포 전 삭제되어 증거가 없는 과거 계좌는 현재 데이터만으로 복원할 수 없다.
