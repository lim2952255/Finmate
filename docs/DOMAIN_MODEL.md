...
- 일반계좌가 없는 사용자가 처음 개설하면 통화와 관계없이 대표계좌로 자동 지정한다. 추가 개설은 대표 선택을 바꾸지 않으며, 이후 사용자가 직접 변경할 수 있다.
- 모든 계좌 엔티티는 잔액 0으로 생성하며, 통화별 1회·일일 이체 한도를 갖는다.
- 일반계좌는 통화를 합산해 사용자당 최대 3개다. 개설 서비스가 첫 원화 계좌에 사용자당 1회 모의자금 100,000,000원을 지급하고 `DEPOSIT` 원장을 남긴다. USD 계좌는 0원이며 먼저 개설해도 지급 자격을 소진하지 않는다.
- 기존 원화 계좌가 있는 사용자는 이전 정책의 시작 자금을 지급받은 것으로 처리하며 추가 지급·잔액 보정을 하지 않는다. 기존 3개 초과 계좌는 유지하되 추가 개설을 거부한다.
...
- 증권계좌가 없는 사용자의 첫 개설 계좌를 대표 증권계좌로 자동 지정한다. 일반계좌 대표 지정과는 독립적이며, 추가 개설은 기존 대표 선택을 유지한다.
- 사용자당 최대 3개이며, 생성 시 `CurrencyCode.values()`의 모든 통화에 대한 `InvestmentCashBalance`를 잔액 0으로 생성한다. 투자금은 본인 일반계좌에서 입금한다.
...
- `StockNewsCache`는 종목별 한 행에 검색어, 선택된 전략 타입, 최종 뉴스 10건 JSON, 생성·갱신 시각을
  저장한다. 현재 설정의 전략 타입과 다른 기존 캐시는 재사용하지 않고 다음 조회에서 갱신한다.
- `MarketReportCache`는 KOSPI, KOSDAQ, NASDAQ, S&P 500, 금리, 환율을 나타내는 `MarketReportTopic`별 한 행에
  검색어, 최종 뉴스 10건 JSON, 생성·갱신 시각을 저장한다.
- 종목 뉴스와 시장 리포트 JSON의 각 `NewsItem`에는 KR-FinBert-SC가 판정한 `POSITIVE`, `NEUTRAL`,
...
- 즉시 반영용 시작 동기화도 별도 설정을 명시적으로 활성화했을 때만 같은 멱등 서비스를 한 번 호출한다.
- 개념 조회 API는 DB에 동기화된 활성 정적 개념과 코드별 SVG를 반환하고, 요청 경로의 종목 ID로
  최신 시세·분기 재무·투자자 수급을 조회해 실제 값, 계산식, 기준시점과 중립적인 해석을 함께 반환한다.
- EPS·BPS처럼 제공기관 산정 기간이나 주식 수 기준이 화면 데이터와 다를 수 있는 값은 임의로 재계산하지
  않으며, 컨센서스처럼 원천 데이터가 없는 항목은 계산하지 않고 데이터 부재를 명시한다.
