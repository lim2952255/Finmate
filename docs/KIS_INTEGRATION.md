...
| 용도 | 경로 | TR ID |
|---|---|---|
| 국내 종목 통합 일·주·월·연봉(KRX+NXT) | `/uapi/domestic-stock/v1/quotations/inquire-daily-itemchartprice` | `FHKST03010100` |
| 국내 종목 일자별 1분봉(KRX+NXT) | `/uapi/domestic-stock/v1/quotations/inquire-time-dailychartprice` | `FHKST03010230` |
| 해외 종목 일·주·월봉 | `/uapi/overseas-price/v1/quotations/dailyprice` | `HHDFS76240000` |
| 해외 종목 1분봉 | `/uapi/overseas-price/v1/quotations/inquire-time-itemchartprice` | `HHDFS76950200` |
...
종목 상세 차트는 기본 일봉과 주·월·연봉 목록을 제공한다. 일봉은 최근 3년, 주봉은 최근 10년, 월봉과
연봉은 상장 이후 범위를 온디맨드로 동기화한다. 국내는 같은 기간별 차트 API에 `D/W/M/Y`를 전달하고,
해외 개별 종목 API는 `D/W/M`만 지원하므로 NASDAQ 연봉은 저장된 월봉을 연 단위로 집계한다. 확정 일봉은
`stock_daily_price`, 확정 주·월·연봉은 `stock_period_price`에 저장한다. 현재 주·월·연봉은 현재 기간에
속한 확정 일봉으로 임시 캔들을 만들고 KIS WebSocket 체결값으로 OHLCV를 갱신한다.

종목 상세 차트의 `1분봉`, `3분봉`, `5분봉`, `15분봉`은 DB에 저장하지 않는다. 캐시 미스이거나 캐시의
최근 거래일·장중 갱신시각이 현재 시장 세션을 충족하지 못하면 국내 종목은
일자별 분봉 API로 최근 표시 대상 거래일의 1분봉을 조회하고, NASDAQ 종목은 해외주식분봉조회 API에서
전일 포함 1분봉을 조회한다. 원본 1분봉은 `stock:minute-candles:{stockId}` Redis 키에 JSON으로 저장하며
기본 TTL은 3일이다. 일반적인 주말은 캐시로 넘기고 더 긴 휴장 뒤에는 첫 화면 조회에서 다시 KIS를 호출한다.
Redis 장애 시에는 같은 프로세스의 JVM 캐시를 fallback으로 사용한다.
3·5·15분봉은 별도로 저장하거나 KIS에 다시 요청하지 않고 캐시된 1분봉 OHLCV를 서버에서 집계한다.

Redis에는 KIS REST로 조회한 1분봉 스냅샷을 저장하고, 캐시가 현재 시장 세션 기준으로 오래된 경우 차트 조회 시
다시 불러온다. 브라우저는 KIS WebSocket 체결 메시지의 체결시각·체결가·체결량으로 선택한 분봉의 현재 봉을
실시간 갱신하며, 새 분으로 넘어가면 직전 봉을 확정하고 새 미확정 봉을 추가한다. 장중에는 캐시에 직전 거래일과 오늘 데이터가
있으면 두 거래일을 이어 표시하고, 장 마감·주말·휴장 상태에서는 캐시에 존재하는 마지막 거래일만 표시한다.
거래일 사이에는 실제 체결이 없는 가짜 봉을 만들지 않는다.

국내 일자별 분봉과 해외주식 분봉 API는 실전 환경 전용이다. 국내 일자별 분봉은 호출당 최대 120건이며
입력 시각을 과거로 이동해 한 거래일을 나눠 조회한다. 해외주식 분봉도 호출당 최대 120건이고 `NEXT`와
`KEYB`로 이전 구간을 이어 조회한다. 해외 API가 제공하는 과거 분봉은 정규장 기준이므로 프리·애프터마켓의
현재 봉은 WebSocket 체결을 통해서만 추가될 수 있다.

서버 경계는 선택한 봉의 날짜·시가·고가·저가·종가·거래량·거래대금을 전달하는 데까지다. 차트 좌표와
축, 이동평균선, 보이는 구간의 가격 범위는 브라우저에서 계산한다. 사용자 조작만으로 추가 KIS REST 호출이
발생하지 않는다. 최초 화면에는 최근 63개 봉을 표시하고, 축소하면 선택한 전체 조회 범위를 표시한다.
나머지 데이터는 차트 드래그나 하단 기간 바를 움직여
이동한다. 기간 바는 전체 조회 범위에서 현재 표시 중인 구간의 위치와 폭을 보여주며 확대·축소에 따라 손잡이
폭도 변한다. 보이는 구간의 최고·최저에는 가격, 현재 종가 대비 등락률과 거래일을 표시한다. 커서를 올리면
거래일·요일과 OHLC·거래량·거래대금의 이전 봉 대비 변화율을 함께 보여준다. KIS WebSocket 체결값은
현재 진행 중인 마지막 캔들에 반영한다. 환율·지수 종가 차트도 선택 기간의 최고·최저 종가와 해당 날짜를 차트 안에
표시한다.
...
- 종목 1분봉 원본은 Redis/JVM fallback에 최근 표시 세션만 저장되고 3·5·15분봉은 조회 시 집계된다.
