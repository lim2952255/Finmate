...
import com.finmate.service.account.AccountService;
...
public class AccountController {
    ...
    @PostMapping("/list")
    public String accountsLists(@RequestParam String primary ,Model model
            ,@SessionAttribute(name = Const.LOGIN_USER) SessionUser sessionUser){

        User user = userService.findUser(sessionUser);
        try{
            accountService.setPrimary(primary, user);
        }  catch(Exception e){
            List<Account> accounts = accountService.findAccounts(sessionUser.getId());
            model.addAttribute("accounts", accounts);
    ...
    public String accountsTransection(){
        return "accounts/transection";
    }
}
