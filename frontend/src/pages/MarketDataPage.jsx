...
export function MarketDataIndexPage() {
  return <div className="page"><Header /><main className="main"><section className="content"><div className="page-heading"><span className="eyebrow">MARKET DATA</span><h1>환율 / 지수 정보</h1><p>주요 환율과 국내외 주가지수의 흐름을 확인합니다.</p></div><section><div className="section-title"><h2>조회 메뉴</h2><p>KIS에서 필요한 기간의 일봉을 조회하고 저장해 표시합니다.</p></div><div className="menu market-data-menu"><Link className="menu-item market-data-menu-item" to="/investments/exchanges"><strong>실시간 환율</strong><span>USD/KRW 환율과 기간별 가격 흐름</span><span className="market-data-menu-arrow" aria-hidden="true">환율 차트 보기 <b>→</b></span></Link><Link className="menu-item market-data-menu-item" to="/investments/indices"><strong>실시간 지수 시세</strong><span>KOSPI·KOSDAQ·NASDAQ 지수 흐름</span><span className="market-data-menu-arrow" aria-hidden="true">지수 차트 보기 <b>→</b></span></Link></div><p className="action-row"><Link className="button secondary" to="/investments">투자 홈</Link></p></section></section></main></div>;
}
...
function MarketLineChart({ data, summary }) {
  ...
  const dateTickIndexes = [...new Set([0, 1, 2, 3, 4, 5].map((step) => Math.round((points.length - 1) * step / 5)))];
