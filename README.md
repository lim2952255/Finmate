...
두 이미지는 같은 키워드 결과 화면의 상단과 하단입니다. 결과 번호 03·05·07·08처럼 삼성전자와 SK하이닉스의 장중 상승·반도체 강세를 다룬 기사가 출처와 제목만 달리한 채 Top 10 여러 위치에 반복됩니다. 키워드가 정확히 포함됐다는 점에서는 모두 관련 기사지만, 사용자가 새롭게 얻는 사건 정보는 기사 수만큼 늘어나지 않는다는 것이 기존 방식의 핵심 문제였습니다.

이를 관련성 순위를 유지하면서도 **새로운 사건을 더 많이 보여주는 novelty 선별 문제**로 정의하고 다음 네 전략을 같은 후보 데이터로 비교했습니다.

| 전략 | 중복 판별 방식 | 장점 | 단점 | 실험에서 확인한 특징 |
|---|---|---|---|---|
| `KEYWORD` | 별도의 기사 간 비교 없이 날짜·키워드 점수로 Top 10 선택 | 가장 빠르고 선정 이유를 설명하기 쉬움 | 표현이 비슷한 동일 사건을 제거할 수 없음 | 2.75ms로 가장 단순했지만 중복률이 39.17%로 가장 높음 |
| `TF_IDF` | 일반 토큰 TF-IDF cosine 유사도 | 모델 파일 없이 빠르게 어휘 중복을 줄임 | 한국어 조사·어미와 복합어 변화에 취약 | 0.20에서 최고 nDCG 0.8189를 기록했지만 중복률 20.83%가 남음 |
| `KOREAN_TF_IDF` | Lucene Nori 형태소 TF-IDF cosine 유사도 | 한국어 형태 변화를 반영하면서 운영 비용이 낮음 | 문맥이나 동의어 수준의 의미 유사성에는 한계 | 0.20에서 중복률 13.33%, 관련 고유 사건 5.58건으로 균형이 가장 좋음 |
| `EMBEDDING` | `multilingual-e5-small` 문장 임베딩 cosine 유사도 | 단어가 달라도 의미가 비슷한 기사를 포착 | 모델 배포·초기화가 필요하고 추론 비용이 큼 | 0.92의 품질은 Nori와 비슷했지만 평균 1,803.75ms로 약 46배 느림 |

> **실험 결론:** 운영 전략은 **Lucene Nori 기반 `KOREAN_TF_IDF`**, 중복 판정 cosine 유사도 임계값은 **`0.20`**으로 선택했습니다. 기존 키워드 방식의 관련성 순서를 유지하면서 중복률을 39.17%에서 13.33%로 낮추고, 관련 고유 사건을 평균 3.83건에서 5.58건으로 늘렸습니다.
